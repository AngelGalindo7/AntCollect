...
export const SideBar: React.FC<SideBarProps> = ({ unreadCount = 0, isChatRoute = false }) => {
  ...
  // On chat routes the inline panel in Layout is already visible — the icon
  // becomes a passive highlighted indicator rather than a toggle.
    ...
    <>
      <aside className="relative w-20 bg-gray-100 border-r border-gray-200 flex flex-col h-full z-20">
        <nav className="flex-1 p-4 flex flex-col gap-4">
          ...
          <button
            onClick={isChatRoute ? undefined : () => setMessagesOpen((prev) => !prev)}
      ...
      {/* Sliding panel — only shown on non-chat routes */}
      {!isChatRoute && (
        <div
